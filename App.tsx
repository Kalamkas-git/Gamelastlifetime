...
export default function App() {
  ...
  const maslowData = useMemo(() => {
    ...
    // Исправлено: добавлены пол и возраст в итоговую строку для таблицы
  ...
  const handleChoice = (choice: Choice) => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        if (nextIndex === 3 || nextIndex === 6) { setActJustCompleted(nextIndex / 3); setPhase('ACT_SUMMARY'); setSceneIndex(nextIndex); setIsTransitioning(false); }
        else if (nextIndex >= 9) { setIsMuted(true); setPhase('GAME_OVER_SEQUENCE'); setGameOverStep(0); setIsTransitioning(false); }
        else { setSceneIndex(nextIndex); setIsTransitioning(false); } 
      }, 500);
    }, 100);
  };
  ...
  return (
    <div id="app-container" className="w-full h-full bg-[#050505] overflow-hidden">
      ...
      {phase === 'BIRTH_MOMENT' && <BirthMoment onComplete={() => setPhase('CONSENT')} />}
      {phase === 'CONSENT' && <ConsentScreen onComplete={(agreed) => { setConsentGiven(agreed); setPhase('PROFILE'); }} />}
      {phase === 'PROFILE' && (
        <div className="relative min-h-screen flex items-center justify-center p-6 bg-void-900 text-white font-sans overflow-hidden">
          <Background hour={7} /><div className="relative z-10 w-full max-w-md bg-[#0a0a0a]/90 backdrop-blur-xl border border-[#5c4033] p-8 rounded shadow-2xl animate-fade-in"><div className="text-center mb-8"><User className="w-12 h-12 text-[#d4af37] mx-auto mb-4" /><h2 className="text-2xl font-serif text-white mb-2 uppercase tracking-widest">Личность</h2></div><div className="space-y-6"><div className="space-y-2"><label className="text-[10px] uppercase tracking-[0.2em] text-gray-500 font-bold">Имя</label><input type="text" value={playerProfile.name} onChange={(e) => setPlayerProfile({...playerProfile, name: e.target.value})} placeholder="Впиши имя..." className="w-full bg-[#151515] border border-[#333] rounded text-white px-4 py-3 focus:outline-none focus:border-[#d4af37] transition-all text-lg font-serif" /></div><div className="space-y-2"><label className="text-[10px] uppercase tracking-[0.2em] text-gray-500 font-bold">Возраст</label><input type="number" value={playerProfile.age} onChange={(e) => setPlayerProfile({...playerProfile, age: parseInt(e.target.value) || 0})} className="w-full bg-[#151515] border border-[#333] rounded text-white px-4 py-3 focus:outline-none focus:border-[#d4af37] transition-all font-mono" /></div><div className="space-y-2"><label className="text-[10px] uppercase tracking-[0.2em] text-gray-500 font-bold">Пол</label><div className="grid grid-cols-3 gap-2">{(['male', 'female', 'other'] as Gender[]).map((g) => (<button key={g} onClick={() => setPlayerProfile({...playerProfile, gender: g})} className={`py-3 px-4 text-[10px] uppercase tracking-wider border rounded transition-all duration-300 ${playerProfile.gender === g ? 'bg-[#d4af37] text-black border-[#d4af37] font-bold shadow-[0_0_15px_rgba(212,175,55,0.3)]' : 'bg-[#151515] text-gray-500 border-[#333] hover:border-gray-500'}`}>{g === 'male' ? 'Муж' : g === 'female' ? 'Жен' : 'Иное'}</button>))}</div></div><button onClick={() => {setPhase('PLAYING'); setIsMuted(false);}} disabled={!playerProfile.name} className={`w-full mt-6 py-4 rounded uppercase tracking-[0.2em] text-xs font-bold transition-all duration-300 ${playerProfile.name ? 'bg-[#d4af37] text-black hover:bg-[#ffe4b5] shadow-[0_0_20px_rgba(212,175,55,0.2)]' : 'bg-[#222] text-gray-600 cursor-not-allowed border border-[#333]'}`}>Войти в поток</button></div></div></div>
      )}
      {phase === 'ACT_SUMMARY' && (
        <div className="relative min-h-screen flex items-center justify-center p-6 bg-void-900 text-white font-sans"><Background hour={actJustCompleted === 1 ? 12 : 18} /><div className="relative z-10 max-w-3xl text-center space-y-8 animate-fade-in-up bg-[#0a0a0a]/80 backdrop-blur-md p-12 rounded border-y-4 border-[#5c4033] shadow-2xl"><h2 className="text-3xl md:text-5xl font-serif text-white">Фаза {actJustCompleted === 1 ? 'Утра' : 'Дня'} Завершена</h2><button onClick={() => setPhase('PLAYING')} className="group inline-flex items-center gap-3 px-10 py-4 border border-[#d4af37] text-[#d4af37] uppercase tracking-widest text-xs font-bold hover:bg-[#d4af37] hover:text-black transition-all">Продолжить <ArrowRight size={14} /></button></div></div>
      )}
      {phase === 'GAME_OVER_SEQUENCE' && (
        <div className="relative min-h-screen bg-black text-white font-sans flex flex-col items-center justify-center p-6 md:p-12 overflow-hidden">
          ...
          {gameOverStep === 1 && (<div className="w-full max-w-2xl text-center space-y-8 animate-fade-in"><h3 className="text-xs uppercase tracking-[0.4em] text-[#d4af37]/60">Твой жизненный фундамент</h3><MaslowPyramid scores={maslowData.scores} dominant={maslowData.dominant} /><button onClick={() => setGameOverStep(2)} className="inline-flex items-center gap-2 text-xs uppercase tracking-widest text-[#d4af37] font-bold border-b border-[#d4af37]/30 pb-1">Профиль личности <ArrowRight size={14} /></button></div>)}
